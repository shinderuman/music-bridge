...
var (
	...
	newScriptCommand      = systemScriptCommand
)
...
func systemScriptCommand(name string, args []string, stdout, stderr io.Writer) scriptCommand {
...
// LoadPlaylists returns Music.app playlist summaries, or reads the supplied JSON source.
func LoadPlaylists(source string, summary bool, names []string) ([]Playlist, error) {
	return loadPlaylists(source, summary, names)
}
...
func runMusicScript(args []string) ([]byte, error) {
	...
	cmd := newScriptCommand("osascript", args, &out, os.Stderr)
...
// LoadSyncPlaylists resolves full track metadata while reusing the host cache.
func LoadSyncPlaylists(source string, selected []Playlist, refresh bool) ([]Playlist, error) {
	return loadSyncPlaylists(source, selected, refresh)
}
...
func exportArtworksFromMusic(playlists []Playlist, artworkDir string, requests []ArtworkRequest) error {
	...
	cmd := newScriptCommand("osascript", args, io.Discard, os.Stderr)
	if err := cmd.Start(); err != nil {
